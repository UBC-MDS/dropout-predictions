...
def main(train, out_dir):
    ...
    cross_val_results = output_object(final_model_log, cross_val_results, X_train, y_train)
...
def output_object(best_model, result_dict, train_x, train_y):
    scoring_metrics = 'f1'
    result_dict['logreg'] = pd.DataFrame(cross_validate(best_model, train_x, train_y, return_train_score=True, scoring=scoring_metrics)).agg(['mean', 'std']).round(3).T
